...
export default function PostModal({ post: initialPost, posts, onClose }: PostModalProps) {
  ...
  return (
    ...
    >
      ...
      {/* Modal */}
      <div
        className="bg-ig-surface w-full h-full md:h-auto md:max-h-[90vh] md:rounded-sm flex flex-col md:flex-row md:max-w-233.75 overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Image — desktop: left half; mobile: fixed height top */}
        <div className="bg-black flex items-center justify-center shrink-0 h-[44vw] min-h-60 max-h-90 md:h-auto md:max-h-none md:flex-1 md:min-h-0">
          <img
            src={post.imageUrl}
            alt="post"
        ...
        {/* Panel */}
        <div className="flex-1 md:flex-none md:w-85 flex flex-col border-t border-ig-border md:border-t-0 md:border-l bg-ig-surface overflow-hidden min-h-0">
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 border-b border-ig-border shrink-0">
            <Link
              ...
              onClick={onClose}
            >
              <div className="w-8 h-8 rounded-full overflow-hidden story-gradient p-0.5 shrink-0">
